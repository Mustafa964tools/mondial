  ...
  if (sortedEvents.length === 0) {
    return (
      <div id="timeline-empty-state" className="bg-[#0c101d] rounded-xl border border-white/10 p-6 shadow-xl text-center">
        <p className="text-white/60 text-sm">هیچ ڕووداوێکی تۆمارکراو نییە.</p>
  ...
  return (
    <div id="match-events-timeline" className="bg-[#0c101d] rounded-xl border border-white/10 p-4 md:p-6 shadow-xl">
      <h3 className="text-gold-bright font-bold text-lg border-b border-white/5 pb-2 mb-6">
        ڕووداوە چڕیەکانی یارییەکە <span className="text-white/40 text-xs font-mono">/ Timeline</span>
      </h3>
        ...
        {sortedEvents.map((event) => {
          ...
          return (
            <div key={event.id} className="relative flex items-start gap-4">
              ...
              <div className="flex-1 bg-[#161d30]/30 p-3 rounded-lg border border-white/5 flex items-center justify-between">
                <div>
                  <div className="flex items-center gap-2">
                    {/* Event Icons */}
                    {event.type === 'goal' && (
                      <span className="p-1 rounded bg-amber-500/10 text-gold-bright">
                        <Trophy className="w-4 h-4" />
                      </span>
                    )}
                    {event.type === 'card' && (
                      <span
                        className={`w-3.5 h-5 rounded-sm block ${
                          event.cardType === 'red' ? 'bg-rose-500 shadow-rose-900/30' : 'bg-yellow-400 shadow-yellow-900/30'
                        } border border-black/30 shadow`}
                      />
                    )}
                    {event.type === 'sub' && (
                      <span className="p-1 rounded bg-white/5 text-white/70">
                    ...
                    <div>
                      ...
                      <span className="text-[10px] text-white/40 bg-white/5 px-1 rounded ml-2">
                        {activeTeam.nameKu}
                      </span>
                    </div>
                  </div>
                  ...
                  {event.type === 'goal' && event.assist && (
                    <div className="text-[11px] text-white/50 mt-1 mr-6 font-sans">
                      یارمەتیدەر: <span className="font-medium text-white">{translateTextKu(event.assist)}</span>
                    </div>
                  )}

                  {event.type === 'sub' && (
                    <div className="space-y-0.5 mt-1 mr-6 text-xs font-sans">
